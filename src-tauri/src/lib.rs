...
#[tauri::command]
async fn analyze_stock(symbol: String, download: bool) -> Result<Value, String> {
    // 获取当前工作目录
...
#[tauri::command]
async fn list_reports() -> Result<Value, String> {
    let current_dir = std::env::current_dir().map_err(|e| e.to_string())?;
    let mut data_dir = current_dir.join("data");
...
pub fn run() {
  tauri::Builder::default()
    ...
    .invoke_handler(tauri::generate_handler![analyze_stock, list_reports, open_file])
    .setup(|app| {
